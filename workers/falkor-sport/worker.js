// falkor-sport v1.6.0 — NRL ladder added
...
async function getSportSummary(year,round,pin,env){const[ladder,games,nextRace,nrlLb]=await Promise.allSettled([getAFLLadder(year),getAFLRound(year,round),getRacingNextToJump(),getNRLLeaderboard(pin,env)]);const nrlLeader=nrlLb&&nrlLb.status==="fulfilled"&&Array.isArray(nrlLb.value)&&nrlLb.value.length>0?nrlLb.value[0].player+" ("+nrlLb.value[0].correct_tips+" correct)":null;const top5=ladder.status==="fulfilled"?ladder.value.slice(0,5).map(t=>t.rank+". "+t.team+" ("+t.wins+"W-"+t.losses+"L "+t.points+"pts)").join(", "):"unavailable";const roundGames=games.status==="fulfilled"?games.value:[];const detectedRound=round||(roundGames.length>0?Math.min(...roundGames.map(g=>g.round)):null);const cur=detectedRound?roundGames.filter(g=>g.round===detectedRound):roundGames.slice(0,9);const finals=cur.filter(g=>g.status==="final");const upcoming=cur.filter(g=>g.status==="upcoming");const live=cur.filter(g=>g.status==="live");const resultsText=finals.map(g=>g.home+" "+g.homeScore+" def "+g.away+" "+g.awayScore).join("; ")||"none yet";const upcomingText=upcoming.map(g=>g.home+" v "+g.away).join(", ")||"none";const racingInfo=nextRace.status==="fulfilled"&&nextRace.value.ok?nextRace.value.races.slice(0,2).map(r=>r.meeting+" R"+r.id).join(", "):"TAB unavailable";let compText="";try{const comp=await getFootyComp(year,round,pin,env);if(comp.season.length>0)compText="; Family tipping: "+comp.season.map(p=>p.player+" "+p.correct+"/"+p.total).join(", ");}catch{}return{year,round,summary:"AFL Round "+(detectedRound||"?")+" "+year+" Top 5: "+top5+". Results: "+resultsText+". Upcoming: "+upcomingText+"."+compText+" Racing: "+racingInfo,ladder:ladder.status==="fulfilled"?ladder.value:[],roundGames,currentRound:detectedRound,racing:nextRace.status==="fulfilled"?nextRace.value:{ok:false},nrl_tipping_leader:nrlLeader};}
...
async function getNRLTippingSummary(season, pin, env) {
  const yr = season || new Date().getFullYear();
  const [lb, draw] = await Promise.allSettled([
    getNRLLeaderboard(pin, env),
    getNRLDraw(yr, null, env),
  ]);
  const leaderboard = lb.status === 'fulfilled' ? lb.value : [];
  const rounds = draw.status === 'fulfilled' ? draw.value : [];
  const currentRound = rounds.length > 0
    ? rounds.find(function(g) { return g.status === 'upcoming'; })?.round || rounds[rounds.length-1]?.round || null
    : null;
...
if(path==="/nrl/tipping")return json(await getNRLTippingSummary(parseInt(url.searchParams.get("season")||new Date().getFullYear()),pin,env));
if(path==="/nrl/summary"){const s=parseInt(url.searchParams.get("season")||new Date().getFullYear());const[ladder,tipping]=await Promise.allSettled([getNRLLadder(s,env),getNRLTippingSummary(s,pin,env)]);const top5=(ladder.status==="fulfilled"&&ladder.value.ladder)?ladder.value.ladder.slice(0,5).map(function(t){return (t.position||"?")+". "+(t.team||t.name||"?")+" ("+t.wins+"W-"+t.losses+"L)";}).join(", "):"unavailable";const tippingData=tipping.status==="fulfilled"?tipping.value:{};return json({ok:true,season:s,ladder_top5:top5,tipping:tippingData,leader:tippingData.leader||null});}
