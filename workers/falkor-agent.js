  ...
  async processChat(text, model, ws, productContext, userId, systemPrefix) {
    ...
    const systemPrompt = [
      ...
      `- Full rules: https://github.com/PaddyGallivan/asgard-source/blob/main/docs/ENGINEERING-RULES.md`,
            `## Website Editing Workflow (all production sites):`,
            `When ${userCtx.name} requests edits to any website (Carnival Timing, School Sport Portal, SportCarnival, LessonLab, Bomber Boat, Superleague Yeah, Mascot Generator, Long Range Tipping, School Staff Hub, KBT Trivia Tools, Falkor Widget):`,
            `1. IDENTIFY: Fetch https://raw.githubusercontent.com/Luck-Dragon-Pty-Ltd/asgard-source/main/site-config.json to find repo, deploy type, and source path for the site.`,
            `2. CLONE: Use GitHub token from vault to clone the repo (e.g., git clone https://github.com/Luck-Dragon-Pty-Ltd/sport-carnival.git).`,
            `3. LOCATE: Find the file to edit based on deploy type:`,
            `   - CF Pages: Edit files in public/ or src/ (check site-config source_path)`,
            `   - CF Worker: Edit files in worker.js or src/worker.js`,
            `   - GitHub Pages: Edit files in docs/ or root`,
            `4. EDIT: Make the requested change. Be specific about what changed.`,
            `5. COMMIT: git commit -m "Update: {clear description of change}"`,
            `6. PUSH: git push origin main — webhook auto-deploys (CF Pages ~30s, CF Workers ~10s, GitHub Pages ~2min).`,
            `7. VERIFY: Wait for deploy, then fetch the live URL and confirm the change is visible. Report: "Deployed to {domain} — change live."`,
            `Never bypass this workflow. Never edit via GitHub web UI. Always route through clone → edit → commit → webhook.`,
