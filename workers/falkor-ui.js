...
function VoiceModal({ voiceState, transcript, reply, analyserRef, onMicClick, onClose }) {
  const statusMap = {
    ...
    listening:  '\ud83d\udd34 ListeningÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦',
    processing: '\u2699\ufe0f ProcessingÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦',
    speaking:   '\ud83d\udd0a SpeakingÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦',
  };
  return (
    <div className="voice-overlay">
      <button className="voice-close" onClick={onClose}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ</button>
      <div className="voice-dragon">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ</div>
  ...
  return (
    <div className="login-wrap">
      <div className="login-card">
        <div className="login-logo">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Falkor</div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="field"><label>Password</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢" required /></div>
          <button className="btn" type="submit" disabled={loading}>{loading ? 'Signing inÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦' : 'Sign in'}</button>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <div className="settings-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="settings-panel">
        <div className="settings-title">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Settings</div>
        ...
        <div className="setting-row"><span className="setting-label">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Voice replies</span><div className={'toggle '+(voiceEnabled?'on':'')} onClick={onVoiceToggle}/></div>
  ...
  return (
    <div style={{flex:1,overflow:'auto',padding:'16px 20px'}}>
      <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'16px',flexWrap:'wrap'}}>
        <span style={{fontSize:'18px',fontWeight:700}}>ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ AFL {YEAR}</span>
      ...
      {loading && <div style={{color:'var(--muted)',fontSize:'13px',padding:'20px'}}>LoadingÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦</div>}
            ...
            <div key={g.id} style={{background:'var(--panel)',border:'1px solid var(--border)',borderRadius:'10px',padding:'12px 16px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'8px',justifyContent:'space-between',flexWrap:'wrap'}}>
                <div style={{display:'flex',gap:'12px',alignItems:'center',flex:1}}>
                  ...
                  <span style={{color:'var(--muted)',fontSize:'12px',flexShrink:0}}>{g.status==='upcoming'?'vs':g.homeScore+' ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ '+g.awayScore}</span>
              ...
              {g.venue&&<div style={{fontSize:'11px',color:'var(--muted)',marginTop:'4px'}}>ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ {g.venue}</div>}
            </div>
          ))}
        </div>
      )}
      {!loading && tab==='tips' && (
        <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
          <div style={{fontSize:'12px',color:'var(--muted)',marginBottom:'4px'}}>AI model tips ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Round {rnd} (Squiggle)</div>
      ...
      {!loading && tab==='comp' && (
        <div>
            ...
            {games.map(g=>{
              ...
              return (
                <div key={g.id} style={{background:'var(--panel)',border:'1px solid var(--border)',borderRadius:'10px',padding:'12px 16px'}}>
                  ...
                  {done&&<div style={{fontSize:'11px',color:'var(--muted)',marginTop:'4px'}}>Final: {g.home} {g.homeScore} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ {g.awayScore} {g.away}</div>}
                </div>
              );
            })}
          </div>
          {comp?.season?.length>0&&(
            <div>
              <div style={{fontWeight:600,marginBottom:'10px',fontSize:'14px'}}>ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Season {YEAR} Leaderboard</div>
              <table className="sport-table">
          ...
          {comp?.season?.length===0&&<div style={{color:'var(--muted)',fontSize:'13px'}}>No tips submitted yet ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ be the first!</div>}
        </div>
      )}
    </div>
  );
...
function MessageBubble({msg}){
  return(
    <div className={'msg-row '+msg.role}>
      <div className="msg-role">{msg.role==='user'?'You':'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Falkor'}</div>
...
function TypingIndicator(){
  return(
    <div className="msg-row assistant">
      <div className="msg-role">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Falkor</div>
...
function App(){
  ...
  useEffect(()=>{
    ...
    recog.onresult=(e)=>{
      ...
      if(t.includes('hey falkor')&&!showVoiceRef.current&&!drivingModeRef.current){
        setShowVoice(true);
        toast('ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Hey! I\'m listeningÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦');
      }
    };
    recog.onerror=()=>{};
  ...
  function sendMessage(text,fileContent){
    ...
    if(wsRef.current?.readyState!==1){toast('ReconnectingÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦');return;}
    ...
    <div className="app">
      ...
      {drivingMode && (
        <div className="driving-overlay">
          <button className="driving-exit" onClick={()=>setDrivingMode(false)}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Exit Driving Mode</button>
          <div className="driving-dragon">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ</div>
          <div className="driving-name">Falkor</div>
          <VoiceWaveform analyserRef={analyserRef} active={voiceState==='listening'||voiceState==='speaking'} />
          <div className="driving-status">{voiceState==='listening'?'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ´ ListeningÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦':voiceState==='processing'?'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ ThinkingÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦':voiceState==='speaking'?'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ SpeakingÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦':'Tap to speak'}</div>
          ...
          <button className={'driving-mic '+(voiceState==='listening'?'listening':voiceState==='speaking'?'speaking':'')} onClick={handleMicClick} disabled={voiceState==='processing'||voiceState==='speaking'}>
            {voiceState==='listening'?'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¹':voiceState==='speaking'?'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ':'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¤'}
          </button>
        </div>
      )}
      ...
      <aside className={'sidebar'+(sidebarOpen?' open':'')}>
        <div className="sidebar-top">
          <span className="logo-text">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Falkor</span>
          <button className="icon-btn" onClick={()=>setSidebarOpen(false)}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ</button>
        </div>
        ...
        <div className="convo-list">
          {convos.map(c=>(
            <div key={c.id} className={'convo-item'+(c.id===activeId?' active':'')} onClick={()=>{setActiveId(c.id);setSidebarOpen(false);}}>
              ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¬ {c.title||'New chat'}
        ...
        <div className="sidebar-footer">
          ...
          <button className="icon-btn" onClick={()=>setShowSettings(true)}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ</button>
        </div>
      </aside>
      ...
      <main className="main">
        <div className="topbar">
          <button className="icon-btn" onClick={()=>setSidebarOpen(true)}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ°</button>
          <span className="topbar-title">{activeConvo?.title||'Falkor'}</span>
          <button className="icon-btn" style={{fontSize:'13px',padding:'4px 10px',background:view==='chat'?'var(--border)':'transparent'}} onClick={()=>setView('chat')}>ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¬</button>
          <button className="icon-btn" style={{fontSize:'13px',padding:'4px 10px',background:view==='sport'?'var(--border)':'transparent'}} onClick={()=>setView('sport')}>ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ</button>
          ...
          <button className={'voice-btn'+(voiceEnabled?' active':'')} style={{background:'none',border:'none',cursor:'pointer',fontSize:'18px',padding:'4px 6px',borderRadius:'6px',color:voiceEnabled?'var(--accent)':'var(--muted)'}} onClick={()=>setShowVoice(true)} title="Voice">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¤</button>
          <button className="icon-btn" style={{fontSize:'16px'}} onClick={()=>setDrivingMode(true)} title="Driving mode">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ</button>
          <button className="icon-btn" onClick={()=>setShowSettings(true)}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ</button>
        </div>
        ...
        {view==='chat' && (
          <>
            {!activeConvo ? (
              <div className="empty">
                <div className="empty-icon">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ</div>
            ...
            <div className="composer">
              {attachment && (
                <div className="attach-row">
                  ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ {attachment.name}
                  <span className="attach-remove" onClick={()=>setAttachment(null)}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ</span>
                </div>
              )}
              <div className={'composer-inner'+(dragOver?' drag-over':'')} onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onDrop={handleDrop}>
                <textarea ref={taRef} value={input} onChange={e=>setInput(e.target.value)}
                  ...
                  placeholder="Message FalkorÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦" rows={1}
                  onInput={e=>{e.target.style.height='auto';e.target.style.height=e.target.scrollHeight+'px';}}
                />
                <input id="file-input" type="file" style={{display:'none'}} onChange={handleFileInput}/>
                <button className="icon-btn" onClick={()=>document.getElementById('file-input').click()} title="Attach">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ</button>
                <button className={'voice-btn'+(voiceEnabled?' active':'')} style={{background:'none',border:'none',cursor:'pointer',fontSize:'18px',padding:'4px 6px',borderRadius:'6px'}} onClick={()=>setShowVoice(true)} title="Voice">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¤</button>
                <button className="send-btn" onClick={doSend} disabled={(!input.trim()&&!attachment)||wsState!=='connected'}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¤</button>
              </div>
            </div>
          </>
        )}
